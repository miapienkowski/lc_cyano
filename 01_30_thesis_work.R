...
p4<- ggplot(newdata4001, aes(x=as.Date(date), y=binmarker2, 
                             color=marker2,size=3, shape=factor(countMarker))) +
  geom_point() + 
  xlab("")+
  ylab("")+
...
#2016
par(mfrow=c(2,4))
plot(adjustedStack[[183]])
plot(adjustedStack[[184]])
plot(adjustedStack[[185]])
plot(adjustedStack[[186]])
plot(adjustedStack[[187]])
plot(adjustedStack[[188]])
plot(adjustedStack[[189]])
plot(adjustedStack[[190]])
...
#       EVERYTHING BELOW IS COPIED INTO ABOVE
#################################################################
######################################################################
########################################################################
...
sum(rc[!is.na(rc)] == 2) # how many in each clump based on clump value
length(unique(rc)) # how many 'clumps'


bloomChart<-adjustedStack[[7]]
bloomChart
bloomChart[ bloomChart >= .001 ] <- 1
bloomChart[ bloomChart < .001 ] <- 0
as.matrix(bloomChart)
as.matrix(rc)

mousey<-stack(rc, bloomChart)

bloomChart[rc == 1] <- 1000


currentRaster<-adjustedStack[[7]]

for(m in 1:length(unique(rc))){
  if(sum(rc[!is.na(rc)] == m) >= 4){
    (reclass_df <- c(m, 1000)) #code blooms to 1000 value
    (reclass_m <- matrix(reclass_df,
                         ncol = 2,
                         byrow = TRUE))
    currentRaster <- reclassify(rc, reclass_m)
    
  }
}

as.matrix(currentRaster)
rc
...
plot<-ggarrange(q,w,e,r,t,y, rows=3, common.legend=TRUE,legend='right') 
annotate_figure(plot, top = text_grob("Bloom Dates - Main Lake", 
                                      face = "bold", size = 14))
